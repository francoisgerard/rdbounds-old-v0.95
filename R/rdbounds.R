#--------------------------------------------------------------------------
#VERSION 0.95 - BETA
#DISCLAIMER: THIS IS A BETA VERSION OF THE CODE AND IS STILL UNDERGOING TESTING.
#WE APPRECIATE YOUR COMMENTS AND ANY ISSUES NOTICED, AT LEONARD.GOFF@COLUMBIA.EDU
#Copyright (C) 2017 Francois Gerard, Leonard Goff, Miikka Rokkanen, & Christoph Rothe.
...
#'
#' This function implements the estimation procedure in Gerard, Rokkanen, and Rothe (2016) to estimate bounds on treatment effects under potential manipulation of the running varible. Returns an \code{rdbounds} object, which can then be passed to \code{\link{rdbounds_summary}} and \code{\link{rdbounds_export}}.
...
#' @param bwsx is a vector of bandwidths in x, respectively for 1) estimation of the discontinuity in the density of x at the cutoff; and 2)local polynomial estimation of conditional means. Expects either a single bandwidth to be used for both or a vector of two.
#' @param bwy is a bandwidth for density estimation of y, implemented if \code{discrete_y=FALSE}. If \code{discrete_y=FALSE} and bwy is omitted, will be set to the sample standard deviation of y.
...
#' @param num_tau_pairs integer number of points to search over in the set of possible values for \eqn{(\tau_0, \tau_1)} in notation of paper, for fuzzy RD estimation. Defaults to 50. If set to 1, the single tau is set to the "rightmost" (t=1) extreme of the set \eqn{T} (Section 3.1 of paper), such that user can enforce the assumption that always-assigned units always receive treatment ("Refinement B", or Theorem 4 in paper), if this is consistent with data.
#' @param right_effects boolean. If set to \code{TRUE}, additionally estimate causal effects for units just to the right of the cutoff (\eqn{\Gamma_+} or \eqn{\Psi_+(u)} in the notation of the paper, Section 4.3). Defaults to \code{FALSE}.
...
#' @param progressFile a file to output progress messages to (useful if \code{parallelize=TRUE} and the individual cores can't write to screen)
...
#' @examples \donttest{df<-rdbounds_sampledata(30000, covs=TRUE)
...
rdbounds <- function(y, x, covs = NULL, treatment = NULL, c = 0,
                 discrete_x=FALSE, discrete_y=FALSE, bwsx=NULL, bwy=NULL,
                 kernel = "triangular", orders = array(1,dim=c(2)),
                 evaluation_ys=NULL, ymin=NULL, ymax=NULL,
                 type="ate", percentiles=NULL,
                 num_tau_pairs=50, right_effects=FALSE, yextremes=NULL, num_lambdas=50,
                 num_bootstraps = c(100,20), Kn=NULL, alpha=0.05, potential_taus=NULL, parallelize=TRUE, progressFile=NULL,
                 kernel_y=NULL, bwsxcov=NULL, bwycov=NULL, CDFinputs=list(original=NULL, CIsetup=NULL)){

  #INTERPERATE AND CHECK PARAMETERS
  #------------------------------------------------------------------
    set.seed(1)
    options(warn=1)
    start_time <- Sys.time()
    ...
    #Set bandwidths by Silverman's rule if none specified by user
    if (is.null(bwsx)){
      warning("No bandwidth specified for x estimation by the bwsx parameter. Setting x bandwidths to sample standard deviation of x. This choice is only optimal if the data is normally distrbiuted.")
      bwsx<-array(sd(x),dim=c(2))
    }
    if (is.null(bwy) & discrete_y==FALSE){
      warning("discrete_y = FALSE but no bandwidth specified for y estimation by the bwy parameter. Setting y bandwidth to sample standard deviation of y. This choice is only optimal if the data is normally distrbiuted.")
      bwy<-sd(y)
    }
  ...
  #------------------------------------------------------------------
    ...
    if(tau_hat>=1){stop("Estimated discontinuity at cutoff is 1 or larger, and computation cannot procede. The function get_allCDFs() can be run with a user-specified value of tau, and then fed into get_estimates_by_TE()")}
    if(tau_hat==0){stop("No evidence of manipulation found, thus bounds will not differ from estimation that assumes no manipulation")}
  ...
  #------------------------------------------------------------------
    ...
    print_message(paste0("Estimation complete, time taken: ",time_taken), progressFile=progressFile)
...
#------------------------------------------------------------------
#' @export
bounds_estimate <- function(fuzzy=TRUE, type="ate", tau_hat=NULL, num_tau_pairs, tau1s, tau0s, CDFs, u=NULL, isCovEstimate=FALSE, isRightEstimate=FALSE){
  ...
  if(fuzzy & !isCovEstimate & !isRightEstimate){
  ...
  if(fuzzy & !isCovEstimate & !isRightEstimate){
    if(tau0s[num_tau_pairs]>0){
      warning("The data are inconsistent with tau0=0 so refinement B will not be computed")
  ...
  if(fuzzy & isRightEstimate){
    if(tau0s[[num_tau_pairs]]$tau0>0 | tau0s[[num_tau_pairs]]$lambda>0){
      warning("The data are inconsistent with tau0=0 so refinement C will not be computed")
      ...
      #Since "tau1s" is ordered by increasing lambda, and increasing tau1 (decreasing tau0) within each value of lambda, the [[num_tau_pairs]] element of the gammaplus lists indexed by t contains the values for tau0=0, lambda=0
...
#------------------------------------------------------------------
get_estimates_by_TE <- function(allCDFs, fuzzy, gotCovs, covs, right_effects, yextremes, num_lambdas, percentiles){
  ...
  results_by_TE <-sapply(
    ...
    function(u)
    {
      ...
      estimates_main<-bounds_estimate(fuzzy=fuzzy, type=type, tau_hat=tau_hat, num_tau_pairs=num_tau_pairs, tau1s=tau1s, tau0s=tau0s, CDFs=allCDFs$CDFs, u=u)
      ...
      if(gotCovs){
        cov_estimates<-bounds_estimate(fuzzy=fuzzy, type=type, num_tau_pairs=num_tau_pairs, tau1s=allCDFs$covCDFs$tau1s, tau0s=allCDFs$covCDFs$tau0s, CDFs=allCDFs$covCDFs$CDFs, u=u, isCovEstimate=TRUE, isRightEstimate=FALSE)
      ...
      #------------------------------------------------------------------
      estimates_rightside = list(TE_SRD_right_bounds=c(NA,NA), TE_FRD_right_bounds=c(NA,NA),TE_FRD_right_bounds_refinementC=c(NA,NA), TE_components_rightside=NA, TE_outcomes_rightside=NA)
      if(right_effects){
        right_estimates<-bounds_estimate(fuzzy=fuzzy, type=type, num_tau_pairs=num_tau_pairs, tau1s=allCDFs$rightCDFs$tau1s, tau0s=allCDFs$rightCDFs$tau0s, CDFs=allCDFs$rightCDFs$CDFs, u=u, isCovEstimate=FALSE, isRightEstimate=TRUE)
...
compute_CDFinputs <- function(y, dist_cut, fuzzy, treatment, polynomial, W, little_ys, discrete_y, bwy, kernel_y, yboundaries){
  ...
  inputs<-as.data.frame(rbindlist(lapply(
    ...
    function(little_y)
    {
      ...
      if(fuzzy & sum((dist_cut>=0)&(treatment==0))>0){
      ...
      if(fuzzy & sum((dist_cut<0)&(treatment==1))>0){
  ...
  if(!discrete_y & fuzzy){
    densities<-as.data.frame(rbindlist(lapply(
      ...
      function(little_y)
      {
        ...
        kernel_y<-Kh(h=bwy, kernel=kernel_y, x=(y-little_y))/kernel_coverage

        dens_left_untreated<-as.numeric(lm(kernel_y~polynomial, weights=W, subset=((dist_cut<0)&(treatment==0)))$coefficients["(Intercept)"])

        if(fuzzy & sum((dist_cut>=0)&(treatment==0))>0){
          dens_right_untreated<-as.numeric(lm(kernel_y~polynomial, weights=W, subset=((dist_cut>=0)&(treatment==0)))$coefficients["(Intercept)"])
...
#COMPUTE CDFS
#------------------------------------------------------------------
estimateCDFs <- function(inputs=NULL, tau_hat, treatLeft, treatRight, y, dist_cut, fuzzy, treatment, polynomial, W, num_tau_pairs, tau1s, tau0s, little_ys, discrete_y, bwy, kernel_y, yboundaries, isBs=0, isTau="estimated", isCov=NULL){
  if(is.null(isBs)){isBs<-0}
  if(is.null(isTau)){isTau<-"estimated"}
  if(is.null(isCov)){
    isCov<-""
  } else{
    isCov = paste0(", covariate=", isCov)
  }
  ...
  if(fuzzy){
    ...
    #Bounds for Y1
      ...
      if(sum((dist_cut<0)&(treatment==1))==0){
        ...
        if(mean(inputs$G!=pmin(pmax(inputs$G,0),1))>.02){warning(paste0("Warning from bootstrap: ", isBs, ", tau=", isTau, isCov, ": G(y) should be a CDF, but it contains more than 1% of values outside of the unit interval, and no correction has been applied."))}
        if(!all(pmax(inputs$G,0)==sort(pmax(inputs$G,0)))){warning(paste0("Warning from bootstrap: ", isBs, ", tau=", isTau, isCov, ": G(y) should be a CDF, but is not monotonic. This could be evidence against the model, and no correction has been applied."))}
      }
    ...
    #Bounds for Y0
      if(sum((dist_cut>=0)&(treatment==0))==0){
        ...
        if(!discrete_y){s_notau_mass<-pdf2cdf(little_ys=little_ys, pdfy = s_notau, integral_only=TRUE)} else{s_notau_mass<-pmf2cdf(pmfy=s_notau, integral_only=TRUE)}
          ...
          #FOR NOW: correct G()
          s_notau<-s_notau/s_notau_mass
          if(!discrete_y){s_notau_mass<-pdf2cdf(little_ys=little_ys, pdfy = s_notau, integral_only=TRUE)} else{s_notau_mass<-pmf2cdf(pmfy=s_notau, integral_only=TRUE)}

        #Warnings/checks on s_notau
          if(tau_hat <= (treatRight-treatLeft)/(1-treatLeft)){
            if(!discrete_y & s_notau_mass < max(1-tau0s)-.00000001){stop(paste0("Error from bootstrap: ", isBs, ", tau=", isTau, isCov, ": s(y)*(1-tau0) integrates to ", round(s_notau_mass, digits=3), " which is smaller than 1-tau0 for some values of tau0 in the identified set. Computation can not proceed with the full set of possible values of tau0. This could be evidence against the model, or sample limitations. Point estimates can be obtained with a customized set of possible tau0 and tau1 values by passing these as arguments to the function estimateCDFs(), and the resulting CDFs then passed to the function get_estimates_by_TE()."))}
            if(discrete_y & s_notau_mass < max(1-tau0s)-.00000001){stop(paste0("Error from bootstrap: ", isBs, ", tau=", isTau, isCov, ": s(y)*(1-tau0) integrates to ", round(s_notau_mass, digits=3), " which is smaller than 1-tau0 for some values of tau0 in the identified set. Computation can not proceed with the full set of possible values of tau0. This could be evidence against the model, or sample limitations.  Point estimates can be obtained with a customized set of possible tau0 and tau1 values by passing these as arguments to the function estimateCDFs(), and the resulting CDFs then passed to the function get_estimates_by_TE()."))}

            if(!discrete_y & abs(s_notau_mass-1)>.05){warning(paste0("Warning from bootstrap: ", isBs, ", tau=", isTau, isCov, ": s(y)*(1-tau0) should integrate to unity, but instead integrates to ", round(s_notau_mass, digits=3)))}
            if(discrete_y & abs(s_notau_mass-1)>.05){warning(paste0("Warning from bootstrap: ", isBs, ", tau=", isTau, isCov, ": s(y)*(1-tau0) should sum to unity, but instead sums to ", round(s_notau_mass, digits=3)))}
          }
          else{
            warning("Warning from bootstrap: ", isBs, ", tau=", isTau, isCov, ": tau is greater than (g^+-g^-)/(1-g^-), thus the quantity s() may not integrate to unity although it is being treated as a density function in computation.")
          }
          ...
          function(t)
          {
            if(tau0s[t]==1){
              ...
              if(tau0s[t]==0){
                ...
                #Choosing extreme cases for F_NO from s_notau. We have to be careful here since s_notau may not be exactly normalized to have mass of one..

                  #The following line would be: F_N0_L<-trimLeft(F,tau0s[t]) if F were normalized to have exactly mass one, but is slightly more complicated in general
                  #For F_NO_L, want to truncate s_notau from the y-value QL with mass 1-tau0 to its right, i.e. the s_notau_mass-(1-tau0) quantile.
                  #But trimLeft will give it a factor of (1-(s_notau_mass-(1-tau0)))^(-1) rather than (1-tau0)^(-1), so need to renormalize
                  F_N0_L<-trimLeft(F,s_notau_mass-(1-tau0s[t]))*(2-s_notau_mass-tau0s[t])/(1-tau0s[t])

                  #F_N0_U remains straightforward even if F not normalized. Truncate s_notau from QU with mass 1-tau0 to it's left, i.e. the 1-tau0 quantile of F
                  F_N0_U<-trimRight(F,tau0s[t])
              }

              F_Y0_FRD_L<-(inputs$F_left_untreated-N0_weights[t]*F_N0_L)/(1-N0_weights[t])
              F_Y0_FRD_U<-(inputs$F_left_untreated-N0_weights[t]*F_N0_U)/(1-N0_weights[t])
            }
  ...
  return(list(F_Y0_SRD=F_Y0_SRD,F_Y1_SRD=F_Y1_SRD,F_Y0_FRD_t=F_Y0_FRD_t,F_Y1_FRD_t=F_Y1_FRD_t, naive_Fs=naive_Fs, inputs=inputs))
}

#ESTIMATION OF CDF BOUNDS WITH COVARIATE REFINEMENT
#------------------------------------------------------------------
get_covCDFs <- function(covinputs=NULL, type, covs, tau1s, tau0s, treatLefts, treatRights, tau_w, y, dist_cut, fuzzy, treatment, polynomial, order, W, Wcov, discrete_y, bwy, bwycov, kernel_y, yboundaries, num_tau_pairs, CDFs, little_ys, u, progressFile, kernel){
  ...
  pw<-unlist(lapply(
    ...
    function(w)
    {
      ...
      if(probLimit<(-0.00000001)){
        warning(paste0("Warning: estimated left limit of prob(covs=w|x) at the cutoff for w=",w, " is ", round(probLimit,digits=8), " < 0 , replacing with zero"))
        probLimit<-0
      }
      if(probLimit>1.00000001){
        warning(paste0("Warning: estimated left limit of prob(covs=w|x) at the cutoff for w=",w," is ", round(probLimit,digits=8), " > 1, replacing with one"))
        probLimit<-1
      }
      probLimit
    }
  ...
  if(fuzzy){
      ...
      function(i){
       Pi_minus_w<-unlist(lapply(1:num_levels,
           function(w)
           {
             ...
             if(Pi_minus<0){
               warning(paste0("Warning: estimated proportion of potentially-assigned compliers at the cutoff conditional on w=",w," is negative, replacing with zero"))
               Pi_minus<-0
             }
             if(Pi_minus>1){
               warning(paste0("Warning: estimated proportion of potentially-assigned compliers at the cutoff conditional on w=",w," is greater than unity, replacing with one"))
               Pi_minus<-1
             }
             Pi_minus
           }
    ...
    omegas<-lapply(1:num_combos, function(i) omega_numerators[[i]]/(omega_numerators[[i]]%*%pw)) #normalize
...
#------------------------------------------------------------------
get_gammaPlusCDFs <- function(mainCDFs, yextremes, num_lambdas, num_tau_pairs, tau_hat, tau1s, tau0s, treatLeft, treatRight, little_ys, progressFile, fuzzy){
...
#------------------------------------------------------------------
get_allCDFs <- function(CDFinputs=NULL, tau_hat, y, dist_cut, fuzzy, treatment, polynomial, W, Wcov, num_tau_pairs, little_ys, discrete_y, discrete_x, gotCovs, covs, right_effects, yextremes, bwy, bwycov, kernel_y, yboundaries, num_lambdas, isBs=NULL, isTau=NULL, isStar=FALSE, orders, bwsx, bwsxcov, kernel, tau_w_fixed=NULL, progressFile){

  #DEFINE GRID OF TAU1s/TAU0s, ESTIMATE CDFS
  #------------------------------------------------------------------
  prelims<-get_taugrid(tau_hat=tau_hat, y=y, dist_cut=dist_cut, fuzzy=fuzzy, treatment=treatment, polynomial=polynomial, W=W, Wcov=Wcov, yextremes=yextremes, discrete_y=discrete_y, num_tau_pairs=num_tau_pairs, gotCovs=gotCovs, covs=covs, discrete_x=discrete_x, orders=orders, kernel=kernel, bwsx=bwsx, bwsxcov=bwsxcov, tau_w_fixed=tau_w_fixed)
  ...
  #Main CDFs
  CDFs<-estimateCDFs(inputs=CDFinputs$inputs, tau_hat=tau_hat, treatLeft=treatLeft, treatRight=treatRight, y=y, dist_cut=dist_cut, fuzzy=fuzzy, treatment=treatment, polynomial=polynomial, W=W, num_tau_pairs=num_tau_pairs, tau1s=tau1s, tau0s=tau0s, little_ys=little_ys, discrete_y=discrete_y, bwy=bwy, kernel_y=kernel_y, yboundaries=yboundaries, isBs=isBs, isTau=isTau)
  ...
  if(gotCovs){
    num_levels<-length(levels(covs))
    tau_covs<-as.numeric(lapply(1:num_levels, function(w) prelims$bycov[[w]]$tau_hat));tau0s_covs<-lapply(1:num_levels, function(w) prelims$bycov[[w]]$tau0);tau1s_covs<-lapply(1:num_levels, function(w) prelims$bycov[[w]]$tau1)
    treatLeft_covs<-as.numeric(lapply(1:num_levels, function(w) prelims$bycov[[w]]$treatLeft));treatRight_covs<-as.numeric(lapply(1:num_levels, function(w) prelims$bycov[[w]]$treatRight))
    covCDFs<-get_covCDFs(CDFinputs$covinputs, type=type, covs=covs, tau1s=tau1s_covs, tau0s=tau0s_covs, treatLefts=treatLeft_covs, treatRights=treatRight_covs, tau_w=tau_covs, y=y, dist_cut=dist_cut, fuzzy=fuzzy, treatment=treatment, polynomial=polynomial, order=orders[2], W=W, Wcov=Wcov, discrete_y=discrete_y, bwy=bwy, bwycov=bwycov, kernel_y=kernel_y, yboundaries=yboundaries, num_tau_pairs=num_tau_pairs, CDFs=CDFs, little_ys=little_ys, progressFile=progressFile, kernel=kernel)
    if(saveinputs){CDFinputs$covinputs=covCDFs$covinputs}
  }

  #CDFs for "right side" cDFs to estimate Gamma_plus
  rightCDFs<-NA
  if(right_effects){rightCDFs<-get_gammaPlusCDFs(mainCDFs=CDFs, yextremes=yextremes, num_lambdas=num_lambdas, num_tau_pairs=num_tau_pairs, tau_hat=tau_hat, tau1s=tau1s, tau0s=tau0s, treatLeft=treatLeft, treatRight=treatRight, little_ys=little_ys, progressFile=progressFile, fuzzy=fuzzy)}
...
CIs_setup<-function(tau_tilde, dist_cut, num_bootstraps, alpha, Kn, discrete_x, orders, bwsx, bwsxcov, kernel, gotCovs, covs){
  ...
  tau_tildes<-tau_tilde_stars-tau_tilde+max(tau_tilde,Kn*sd_tau)
  tau_hats_b<-unlist(lapply(1:B, function(b) max(0,tau_tildes[b])))

  if(gotCovs){
    tau_tilde_stars_w<-sapply(levels(covs), function(w) unlist(lapply(1:B, function(b) compute_tau_tilde(kernel=kernel, order=orders[1], x=dist_cut[unlist(resample_indices[b])][covs==w], bwsx=bwsxcov, discrete_x=discrete_x))), USE.NAMES=TRUE)
...
#GENERATE CONFIDENCE INTERVALS BY NONPARAMETERIC BOOTSTRAP
#------------------------------------------------------------------
CIs_estimate <- function(CDFinputs=NULL, parallelize=FALSE, fuzzy, gotCovs, covs, right_effects, yextremes, discrete_y, num_tau_pairs, num_lambdas, percentiles, tau_tilde, y, dist_cut, treatment, polynomial, W, Wcov, num_bootstraps, alpha, Kn, discrete_x, orders, bwsx, bwsxcov, kernel, little_ys, bwy, bwycov, kernel_y, yboundaries, potential_taus, takeup_increase, progressFile){
  ...
  allCDFs_star<-get_allCDFs(CDFinputs=CDFinputs$original, isStar=TRUE, tau_w_fixed=tau_star_w, tau_hat=tau_star, y=y, dist_cut=dist_cut, fuzzy = fuzzy, treatment=treatment, polynomial=polynomial, W=W, Wcov=Wcov, num_tau_pairs=num_tau_pairs, little_ys=little_ys, discrete_y=discrete_y, discrete_x=discrete_x, gotCovs=gotCovs, covs=covs, right_effects=right_effects, yextremes=yextremes, num_lambdas=num_lambdas, bwy=bwy, bwycov=bwycov, kernel_y=kernel_y, yboundaries=yboundaries, orders=orders, bwsx=bwsx, bwsxcov=bwsxcov, kernel=kernel, progressFile=progressFile)
  estimates_star_u<-get_estimates_by_TE(allCDFs=allCDFs_star, fuzzy=fuzzy, gotCovs=gotCovs, covs=covs, right_effects=right_effects, yextremes=yextremes, num_lambdas=num_lambdas, percentiles=percentiles)
  ...
  if(parallelize){
    ...
    parallel::clusterExport(clus, envir=environment(), varlist=list("K", "Kh", "CDFinputs", "tau_hats_w", "tau_hats_b", "resample_indices", "y", "dist_cut", "orders", "W", "num_tau_pairs", "little_ys", "discrete_y", "discrete_x", "gotCovs", "covs", "right_effects", "yextremes", "bwy", "kernel_y", "num_lambdas", "bwsx", "kernel", "get_allCDFs", "get_taugrid", "generate_polynomial_matrix", "estimateCDFs", "compute_CDFinputs", "rbindlist", "trimLeft", "trimRight", "pdf2cdf", "pmf2cdf", "cdf2expectation", "get_covCDFs", "get_gammaPlusCDFs", "get_estimates_by_TE",  "naive_estimate", "bounds_estimate", "print_message", "fuzzy", "yboundaries", "integrate_kernel", "Wcov", "bwsxcov", "bwycov"))
    ...
    computations_by_b<-parallel::parLapply(clus, 1:B, function(b){
      this_b_allCDFs<-get_allCDFs(CDFinputs=CDFinputs$bootstraps[[b]], tau_w_fixed=tau_hats_w[b,],tau_hat=tau_hats_b[b],y=y[unlist(resample_indices[b])], dist_cut=dist_cut[unlist(resample_indices[b])], fuzzy = fuzzy, treatment=treatment[unlist(resample_indices[b])], polynomial=generate_polynomial_matrix(dist_cut[unlist(resample_indices[b])],orders[2]), W=W[unlist(resample_indices[b])],  Wcov=Wcov[unlist(resample_indices[b])], num_tau_pairs=num_tau_pairs, little_ys=little_ys, discrete_y=discrete_y, discrete_x=discrete_x, gotCovs=gotCovs, covs=covs, right_effects=right_effects, yextremes=yextremes, num_lambdas=num_lambdas, bwy=bwy, bwycov=bwycov, kernel_y=kernel_y, yboundaries=yboundaries, isBs=b, orders=orders, bwsx=bwsx, bwsxcov=bwsxcov, kernel=kernel, progressFile=progressFile)
      this_b_estimates<-get_estimates_by_TE(allCDFs=this_b_allCDFs, fuzzy=fuzzy, gotCovs=gotCovs, covs=covs, right_effects=right_effects, yextremes=yextremes, num_lambdas=num_lambdas, percentiles=percentiles)
  ...
  #Generate estimates for fixed tau in both original and bootstrap samples
  if(!is.null(potential_taus)){
    allCDFs_tau_star<-lapply(1:length(potential_taus), function(tau) get_allCDFs(CDFinputs=CDFinputs$original, isStar=TRUE, tau_hat=potential_taus[tau], y=y, dist_cut=dist_cut, fuzzy = fuzzy, treatment=treatment, polynomial=polynomial, W=W, num_tau_pairs=num_tau_pairs, little_ys=little_ys, discrete_y=discrete_y, discrete_x=discrete_x, gotCovs=FALSE, right_effects=FALSE, yextremes=yextremes, num_lambdas=num_lambdas, bwy=bwy, kernel_y=kernel_y, yboundaries=yboundaries, orders=orders, bwsx=bwsx, kernel=kernel, isTau=potential_taus[tau], progressFile=progressFile))
    estimates_tau_star<-lapply(1:length(potential_taus), function(tau) get_estimates_by_TE(allCDFs=allCDFs_tau_star[[tau]], fuzzy=fuzzy, gotCovs=FALSE, right_effects=FALSE, yextremes=yextremes, num_lambdas=num_lambdas, percentiles=percentiles))

    if(parallelize){
      clus <- parallel::makeCluster(parallel::detectCores()-1,outfile="")
      tauBcombos<-expand.grid(1:length(potential_taus), 1:B)
      num_tauBcombos<-dim(tauBcombos)[1]
      parallel::clusterExport(clus, envir=environment(), varlist=list("K", "Kh", "tauBcombos", "potential_taus", "CDFinputs", "tau_hats_w", "tau_hats_b", "resample_indices", "y", "dist_cut", "orders", "W", "num_tau_pairs", "little_ys", "discrete_y", "discrete_x", "gotCovs", "covs", "right_effects", "yextremes", "bwy", "kernel_y", "num_lambdas", "bwsx", "kernel", "get_allCDFs", "get_taugrid", "generate_polynomial_matrix", "estimateCDFs", "compute_CDFinputs", "rbindlist", "trimLeft", "trimRight", "pdf2cdf", "pmf2cdf", "cdf2expectation", "get_covCDFs", "get_gammaPlusCDFs", "get_estimates_by_TE", "naive_estimate", "bounds_estimate", "print_message", "fuzzy", "yboundaries", "integrate_kernel", "Wcov", "bwsxcov", "bwycov"))
      ...
      estimates_tauBcombos<-parallel::parLapply(clus, 1:num_tauBcombos, function(i){
        tau<-tauBcombos[i,1]
        b<-tauBcombos[i,2]
        this_allCDFs<-get_allCDFs(CDFinputs=CDFinputs$bootstraps[[b]], tau_hat=potential_taus[tau], y=y[unlist(resample_indices[b])], dist_cut=dist_cut[unlist(resample_indices[b])], fuzzy = fuzzy, treatment=treatment[unlist(resample_indices[b])], polynomial=generate_polynomial_matrix(dist_cut[unlist(resample_indices[b])],orders[2]), W=W[unlist(resample_indices[b])], Wcov=Wcov[unlist(resample_indices[b])], num_tau_pairs=num_tau_pairs, little_ys=little_ys, discrete_y=discrete_y, discrete_x=discrete_x, gotCovs=FALSE, right_effects=FALSE, yextremes=yextremes, num_lambdas=num_lambdas, bwy=bwy, bwycov=bwycov, kernel_y=kernel_y, yboundaries=yboundaries, isBs=b, isTau=potential_taus[tau], orders=orders, bwsx=bwsx, bwsxcov=bwsxcov, kernel=kernel, progressFile=progressFile)
        get_estimates_by_TE(allCDFs=this_allCDFs, fuzzy=fuzzy, gotCovs=FALSE, right_effects=FALSE, yextremes=yextremes, num_lambdas=num_lambdas, percentiles=percentiles)
      })
      parallel::stopCluster(clus)
  ...
  CIs_by_TE <-sapply(
    ...
    function(i)
    {
      ...
      #CI's for naive estimates
        ...
        TE_SRD_naive_CI = c(estimates_star_u[['estimates_naive',i]]$TE_SRD_naive-qnorm(1-alpha/2)*sd(TE_SRD_naives),estimates_star_u[['estimates_naive',i]]$TE_SRD_naive+qnorm(1-alpha/2)*sd(TE_SRD_naives))
        TE_FRD_naive_CI = c(estimates_star_u[['estimates_naive',i]]$TE_FRD_naive-qnorm(1-alpha/2)*sd(TE_FRD_naives),estimates_star_u[['estimates_naive',i]]$TE_FRD_naive+qnorm(1-alpha/2)*sd(TE_FRD_naives))

        #LTG: Old method based on quantiles. Changed 11/16/16 after feedback
          #old_TE_SRD_naive_CI<-2*estimates_star_u[['estimates_naive',i]]$TE_SRD_naive-as.vector(quantile(TE_SRD_naives, probs = c(1-alpha/2,alpha/2), type=1)) #Note that naive effects are same for estimates_star and actual point estimates, so we can just use estimates_star here
          #old_TE_FRD_naive_CI<-2*estimates_star_u[['estimates_naive',i]]$TE_FRD_naive-as.vector(quantile(TE_FRD_naives, probs = c(1-alpha/2,alpha/2), type=1))
      ...
      sL<-sd(unlist(lapply(1:B, function(b) estimates_by_TE_b[[b]][['estimates_main',i]]$TE_lower_SRD)))
      sU<-sd(unlist(lapply(1:B, function(b) estimates_by_TE_b[[b]][['estimates_main',i]]$TE_upper_SRD)))
      ...
      if(fuzzy){
        sL_t<-unlist(lapply(1:num_tau_pairs, function(t) sd(unlist(lapply(1:B, function(b) data.frame(estimates_by_TE_b[[b]][['estimates_main',i]]$TE_components)$TE_tau1s_lower[t])))))
        sU_t<-unlist(lapply(1:num_tau_pairs, function(t) sd(unlist(lapply(1:B, function(b) data.frame(estimates_by_TE_b[[b]][['estimates_main',i]]$TE_components)$TE_tau1s_upper[t])))))
        ...
        TE_FRD_CI<-c(min(lowerbounds_t), max(upperbounds_t))
        ...
        TE_FRD_refinementA_CI<-c(min(lowerbounds_t), max(upperbounds_t))

        #CI for SRD under manipulation, refinement B
        sL_B<-sd(unlist(lapply(1:B, function(b) estimates_by_TE_b[[b]][['estimates_main',i]]$TE_lower_FRD_refinementB)))
        sU_B<-sd(unlist(lapply(1:B, function(b) estimates_by_TE_b[[b]][['estimates_main',i]]$TE_upper_FRD_refinementB)))
      ...
      if(right_effects){
        ...
        #CI for SRD under manipulation
        sL<-sd(unlist(lapply(1:B, function(b) estimates_by_TE_b[[b]][['estimates_rightside',i]]$TE_SRD_right_bounds[1])))
        sU<-sd(unlist(lapply(1:B, function(b) estimates_by_TE_b[[b]][['estimates_rightside',i]]$TE_SRD_right_bounds[2])))
        ...
        if(fuzzy){
          ...
          sL_t<-unlist(lapply(1:num_tau_pairs_rightside, function(t) sd(unlist(lapply(1:B, function(b) unlist(data.frame(estimates_by_TE_b[[b]][['estimates_rightside',i]]$TE_components_rightside)$TE_tau1s_lower)[t])))))
          sU_t<-unlist(lapply(1:num_tau_pairs_rightside, function(t) sd(unlist(lapply(1:B, function(b) unlist(data.frame(estimates_by_TE_b[[b]][['estimates_rightside',i]]$TE_components_rightside)$TE_tau1s_upper)[t])))))
          ...
          lowerbounds_t<-TE_lower_star_t-r_t*sL_t
          upperbounds_t<-TE_upper_star_t+r_t*sU_t
          TE_FRD_right_CI<-c(min(lowerbounds_t), max(upperbounds_t))

          #CI for SRD under manipulation, refinement C
          sL_C<-sd(unlist(lapply(1:B, function(b) estimates_by_TE_b[[b]][['estimates_rightside',i]]$TE_FRD_right_bounds_refinementC[1])))
          sU_C<-sd(unlist(lapply(1:B, function(b) estimates_by_TE_b[[b]][['estimates_rightside',i]]$TE_FRD_right_bounds_refinementC[2])))
      ...
      if(gotCovs){
        #CI for SRD under manipulation
        sL<-sd(unlist(lapply(1:B, function(b) estimates_by_TE_b[[b]][['estimates_covs',i]]$TE_SRD_covs_bounds[1])))
        sU<-sd(unlist(lapply(1:B, function(b) estimates_by_TE_b[[b]][['estimates_covs',i]]$TE_SRD_covs_bounds[2])))
        ...
        if(fuzzy){
          ...
          sL_t<-unlist(lapply(1:num_tau_pairs_covs, function(t) sd(unlist(lapply(1:B, function(b) unlist(data.frame(estimates_by_TE_b[[b]][['estimates_covs',i]]$TE_components_covariates)$TE_tau1s_lower[[t]]))))))
          sU_t<-unlist(lapply(1:num_tau_pairs_covs, function(t) sd(unlist(lapply(1:B, function(b) unlist(data.frame(estimates_by_TE_b[[b]][['estimates_covs',i]]$TE_components_covariates)$TE_tau1s_upper[[t]]))))))
          ...
          lowerbounds_t<- TE_lower_star_t-r_t*sL_t
          upperbounds_t<- TE_upper_star_t+r_t*sU_t
          TE_FRD_covs_CI<-c(min(lowerbounds_t), max(upperbounds_t))
        }
        else{
      ...
      if(!is.null(potential_taus)){
        ...
        #Sharp
        sL_tau<-unlist(lapply(1:length(potential_taus), function(tau) sd(unlist(lapply(1:B, function(b) estimates_tau_b[[tau]][[b]][['estimates_main',i]]$TE_lower_SRD)))))
        sU_tau<-unlist(lapply(1:length(potential_taus), function(tau) sd(unlist(lapply(1:B, function(b) estimates_tau_b[[tau]][[b]][['estimates_main',i]]$TE_upper_SRD)))))
        ...
        #Fuzzy
        if(fuzzy){
          sL_tau_t<-lapply(1:length(potential_taus), function(tau) unlist(lapply(1:num_tau_pairs, function(t) sd(unlist(lapply(1:B, function(b) data.frame(estimates_tau_b[[tau]][[b]][['estimates_main',i]]$TE_components)$TE_tau1s_lower[t]))))))
          sU_tau_t<-lapply(1:length(potential_taus), function(tau) unlist(lapply(1:num_tau_pairs, function(t) sd(unlist(lapply(1:B, function(b) data.frame(estimates_tau_b[[tau]][[b]][['estimates_main',i]]$TE_components)$TE_tau1s_upper[t]))))))
...
#DEFINE GRID OF TAU1s/TAU0s, ESTIMATE CDFS
#------------------------------------------------------------------
get_taugrid <- function(tau_hat, y, dist_cut, fuzzy, treatment, polynomial, W, Wcov, yextremes, discrete_y, num_tau_pairs, gotCovs, covs=NULL, discrete_x=NULL, orders=NULL, kernel=NULL, bwsx=NULL, bwsxcov=NULL, tau_w_fixed=NULL){
  ...
  #Generate tau0s and tau1s for fuzzy estimation
  #Compute extreme pairs of tau0 and tau1: (tau0L, tau1L) and (tau0U, tau1U)
  ...
  taugrid_bycov<-NA
  if(gotCovs){
    #Compute everything for each value of w

    if(is.null(tau_w_fixed)){
      tau_tilde_w<-sapply(levels(covs), function(w) compute_tau_tilde(kernel=kernel, order=orders[1], x=dist_cut[covs==w], bwsx=bwsxcov, discrete_x=discrete_x), USE.NAMES=TRUE)
      tau_w<-sapply(levels(covs), function(w) max(0,tau_tilde_w[w]), USE.NAMES=TRUE)
    } else{
      tau_w<-tau_w_fixed
    }

    num_levels<-length(levels(covs))
    taugrid_bycov<-lapply(
      levels(covs),
      function(w)
      {
        treatLeft<-as.numeric(lm(treatment~polynomial, weights=Wcov, subset=((dist_cut<0)&(covs==w)))$coefficients["(Intercept)"])
        treatRight<-as.numeric(lm(treatment~polynomial, weights=Wcov, subset=((dist_cut>=0)&(covs==w)))$coefficients["(Intercept)"])
        tau1L<-max(0, 1-(1-tau_w[w])/treatRight)
        tau1U<-min(1-treatLeft/treatRight*(1-tau_w[w]),tau_w[w]/treatRight)
        tau0L<-min(1,tau_w[w]/(1-treatRight))
        tau0U<-max(0,tau_w[w]-(1-tau_w[w])*(treatRight-treatLeft)/(1-treatRight))
        #Define sequence of num_tau_pairs values between extremes, for this value of w
        if(num_tau_pairs>1){
          tau1s<-(0:(num_tau_pairs-1))/(num_tau_pairs-1)*(tau1U-tau1L)+tau1L
          tau0s<-(0:(num_tau_pairs-1))/(num_tau_pairs-1)*(tau0U-tau0L)+tau0L
        }
        ...
        list(tau_hat=tau_w[w], treatLeft=treatLeft, treatRight=treatRight, tau1s=tau1s, tau0s=tau0s)
      }
    )

  }

  return(list(tau_hat=tau_hat, treatLeft=treatLeft, treatRight=treatRight, takeup_increase=takeup_increase, tau1s=tau1s,tau0s=tau0s,bycov=taugrid_bycov))
}
...
compute_tau_tilde_old <- function(kernel,order,x,h){
...
#'  Specifically: \deqn{y=(x+10)/2*treatment*(alwaysassigned=0)+5*treatment*(alwaysassigned=1)+rnorm(n,1))} and y is censored at 0 and 23.
...
#' @examples \donttest{df<-rdbounds_sampledata(30000);}
#' @export

#GENERATE SAMPLE DATA
#------------------------------------------------------------------
rdbounds_sampledata<- function(sample_size=30000, covs=FALSE){
  ...
  n1 = sample_size-n0
  tau<-(n1*2/5)/(n0*dnorm(0, mean = 0, sd = sqrt(5), log = FALSE))
...
generate_subsample<- function(sample_size, covs){
  ...
  y = pmin(23,pmax(0,(x+10)/2+2*treatment*(always_assigned==0)+5*treatment*(always_assigned==1)+rnorm(n,1)))
  ...
  results_u<-sapply(1:length(rdbounds$TEs),
    function(i)
    {
      ...
      tabledf<-data.frame(rbind(
        ...
        list("ITT-Bounds on potentially-assigned with covariates", e$TE_SRD_covs_bounds,e$TE_SRD_covs_CI),
        ...
        list(paste0(type_prefix,"-Bounds on potentially-assigned with covariates"), e$TE_FRD_covs_bounds,e$TE_FRD_covs_CI),
...
rdbounds_export<- function(rdbounds, file_name=NULL, view_it=FALSE) {
  ...
  #NOTE: if not doing RHS and/or covs, need to condition
  ...
  for(u in 1:length(rdbounds$TEs)){
          ...
          lapply(
            ...
            function(tau)
            {
              rbind(list(
                ...
                FRD_refA_Y1_lower=bs_estimates$estimates_tau_star[[tau]][['estimates_main',u]]$TE_outcomes$refinementA$Y1_lower, FRD_refA_Y1_upper=bs_estimates$estimates_tau_star[[tau]][['estimates_main',u]]$TE_outcomes$refinementA$Y1_upper,
                FRD_refA_Y0_lower=bs_estimates$estimates_tau_star[[tau]][['estimates_main',u]]$TE_outcomes$refinementA$Y0_lower, FRD_refA_Y0_upper=bs_estimates$estimates_tau_star[[tau]][['estimates_main',u]]$TE_outcomes$refinementA$Y0_upper,
                FRD_refB_Y1_lower=bs_estimates$estimates_tau_star[[tau]][['estimates_main',u]]$TE_outcomes$refinementB$Y1_lower, FRD_refB_Y1_upper=bs_estimates$estimates_tau_star[[tau]][['estimates_main',u]]$TE_outcomes$refinementB$Y1_upper,
                FRD_refB_Y0_lower=bs_estimates$estimates_tau_star[[tau]][['estimates_main',u]]$TE_outcomes$refinementB$Y0_lower, FRD_refB_Y0_upper=bs_estimates$estimates_tau_star[[tau]][['estimates_main',u]]$TE_outcomes$refinementB$Y0_upper,
              ...
              rbindlist(
                lapply(1:B, function(b) {
                  list(
                    ...
                    FRD_refA_Y1_lower=bs_estimates$estimates_tau_b[[tau]][[b]][['estimates_main',u]]$TE_outcomes$refinementA$Y1_lower, FRD_refA_Y1_upper=bs_estimates$estimates_tau_b[[tau]][[b]][['estimates_main',u]]$TE_outcomes$refinementA$Y1_upper,
                    FRD_refA_Y0_lower=bs_estimates$estimates_tau_b[[tau]][[b]][['estimates_main',u]]$TE_outcomes$refinementA$Y0_lower, FRD_refA_Y0_upper=bs_estimates$estimates_tau_b[[tau]][[b]][['estimates_main',u]]$TE_outcomes$refinementA$Y0_upper,
                    FRD_refB_Y1_lower=bs_estimates$estimates_tau_b[[tau]][[b]][['estimates_main',u]]$TE_outcomes$refinementB$Y1_lower, FRD_refB_Y1_upper=bs_estimates$estimates_tau_b[[tau]][[b]][['estimates_main',u]]$TE_outcomes$refinementB$Y1_upper,
                    FRD_refB_Y0_lower=bs_estimates$estimates_tau_b[[tau]][[b]][['estimates_main',u]]$TE_outcomes$refinementB$Y0_lower, FRD_refB_Y0_upper=bs_estimates$estimates_tau_b[[tau]][[b]][['estimates_main',u]]$TE_outcomes$refinementB$Y0_upper,
                    SRD_plus_Y1_lower=NA, SRD_plus_Y1_upper=NA,
                    SRD_plus_Y0_lower=NA, SRD_plus_Y0_upper=NA,
                    FRD_plus_Y1_lower=NA, FRD_plus_Y1_upper=NA,
                    FRD_plus_Y0_lower=NA, FRD_plus_Y0_upper=NA,
    ...
    if(!is.null(file_name)){
      ...
      resultsdf<-data.frame(rbind(
        ...
        list("ITT-Bounds on potentially-assigned with covariates", e$TE_SRD_covs_bounds[1],e$TE_SRD_covs_bounds[2],e$TE_SRD_covs_CI[1],e$TE_SRD_covs_CI[2]),
        ...
        list(paste0(TE_name,"-Bounds on potentially-assigned with covariates"), e$TE_FRD_covs_bounds[1], e$TE_FRD_covs_bounds[2],e$TE_FRD_covs_CI[1], e$TE_FRD_covs_CI[2]),
